...
void forum::vote(
    ...
    if (proposal_hash.size() > 0) {
        eosio_assert(proposal_hash.size() == 64, "proposal_hash should be 64 hexadecimals characters long (sha256 format).");

        checksum256 actual_proposal_hash;
        compute_proposal_hash(row.title, row.proposal_json, &actual_proposal_hash);

        checksum256 expected_proposal_hash;
        proposal_hash_to_checksum256(proposal_hash, &expected_proposal_hash);

        eosio_assert(actual_proposal_hash == expected_proposal_hash, "provided proposal_hash does not match actual computed proposal_hash.");
    }
